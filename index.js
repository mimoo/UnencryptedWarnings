...
alert = '<div style="box-sizing:border-box;z-index:9999999999;position:fixed;left:0;top:0;width:100%;height:30px;background-color:rgba(255,0,0,.7);' + alert_background + ' color:white; box-shadow:0 0 5px rgba(0,0,0, .8);font-weight:bold; font-size:14px; font-family:Arial; padding:5px;border-bottom:1px black solid;">The connection with this webpage is unsecure. It might have been modified by an active attacker.</div><div style="box-sizing:border-box;height:30px;padding:5px;"></div>';

alert2 = "prout";

pageMod.PageMod({
    include: "http://*",
    contentScript: 'document.body.innerHTML = \'' + alert + '\' + document.body.innerHTML' 
});
